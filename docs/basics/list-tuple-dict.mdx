...
Beware of mixing uppercase and lowercase letters. The methods may not work as
...
"A" is 65 while the lowercase "a" is 97.
...
Lists in python being mutable objects, when we set a list variable to another
...
Be also careful when passing a list as argument to a function. It will modify
the existing list, rather than working in a copy to the list.
...
Python lists are dynamic arrays. When a list of length `n` is created, the
memory allocator allocates more than `n` contiguous locations for the list.
...
A single element tuple is declared by `tup = (4,)`. The reason for this
requirement is that without the trailing comma, the expression `(4)` is
...
We can place a comma after the last item in a python list, tuple, or dictionary.
It could be preferable coding style:
...
### Sets
Sets are hash lists. To lookup a member in a hash list has $O(1)$ complexity,
whereas lookups in a python list has $O(n)$ (n is the length of list).
